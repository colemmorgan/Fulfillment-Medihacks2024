...
const QuestionChoice:React.FC<QuestionChoiceProps> = ({value}) => {
  ...
  const [selectedAnswer, setSelectedAnswer] = useRecoilState(setSelectedAnswerAtom)
  ...
  const handleClick = () => {
    ...
    return (
        ...
        ${revealAnswers ? value === currentProblem?.correctAnswer ? "bg-green-300" : "bg-red-200" : "bg-[#f2f4f5]"}`}>
...
interface AnswerBubbleProps {
  ...
  const AnswerBubble: React.FC<AnswerBubbleProps> = ({ letter, onClick, value}) => {
    const [selectedAnswer, setSelectedAnswer] = useRecoilState(setSelectedAnswerAtom)
